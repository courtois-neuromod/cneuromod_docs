...
The mario dataset explores behavioural and brain correlates of videogame play and includes five participants (sub-01, sub-02, sub-03, sub-05 and sub-06; two females, three males) who played Super Mario Bros (SMB; Nintendo, 1985) inside an MRI scanner. While not a central focus of this study, prior videogame experience may have influenced the results. Among the participants, sub-01 had prior experience with SMB, and two participants (sub-01 and sub-02) reported regular videogame play. One participant (sub-06) reported no videogame experience, except another retro platformer called Shinobi III: Return of the Ninja Master (RotNM; Sega, 1993) she practiced for 10+hours as part of a prior CNeuroMod experiment (shinobi dataset) REF.

Behavioural acquisitions
Videogame interface
The videogame was played on a console emulator using OpenAI’s gym-retro library (Nichol et al., 2018), a Python-based platform supporting emulators for over 10 retro consoles and thousands of games. Built on gym (Brockman et al., 2016), a library designed for reinforcement learning, gym-retro integrates console emulators via the Libretro API (Libretro, n.d.). This setup allows both artificial and human agents to play retro games from saved states and provides access to the game’s rapid-access memory (RAM) through a Python API.
...
Videogame task
SMB is a commercial platform side scroller game released in 1985 by the company Nintendo. The aim of the game is to go right and reach the end of each level, while avoiding a variety of obstacles and enemies. The platormers genre was selected by the CNeuroMod team because gym-retro was developed for a major reinforcement competition centered on this genre (OpenAI, 2018). This game was further selected for its compatibility with gym-retro and its commercial success, ensuring engaging gameplay but also extensive prior art investigating the design of the game REF. As in other platformers, players navigate a linear two-dimensional environment filled with enemies and obstacles to reach a destination. The player may collect some power ups which change the form and behaviour of the character in the game. For this study, 22 levels were selected for their gameplay similarity. Specifically, all original levels of SMB were used with the exception of the water and boss levels, which feature very different game mechanics than the other.
Each attempt at completing a level began with no power up and three lives. The attempt was completed if the player reached the end of the level or lost all three lives trying. When players died, they resumed their attempt from the start of the level, or from a dedicated spawn point only featured in some levels and selected by the original game designers. 
Dataset						
We used the Mario dataset made publicly available by the Courtois NeuroMod project (Boyle et al. 2020) (https://docs.cneuromod.ca/en/latest/). The dataset comprises ~15h of fMRI data per subject, for 5 subjects playing 22 Super Mario Bros (SMB) levels. The SMB game is a commercial platform side scroller game released in 1985 by the company Nintendo. The aim of the game is to go right and reach the end of each level, while avoiding a variety of obstacles and enemies. The fMRI data were acquired on a 3T Siemens Prisma Fit scanner (temporal resolution TR=1.49 s, 2 mm isotropic spatial resolution), and preprocessed using the fMRIprep pipeline (Esteban et al. 2019) version XX. , and projected onto MIST atlas, at the 1097 parcels scale (Urchs et al. 2019). In addition to the fMRI data, the button presses from the players and the video frames of the gameplay are acquired. The fMRI, button presses and frame data pertaining to 20 of the 22 levels were split into training, validation and test sets. The training, validation and test sets represent respectively 80%, 10% and 10% of the game runs, with similar proportions of runs from each of the 20 levels in all sets, and with similar proportions of completed/failed runs. A game run corresponds to a game of 3 lives in one level, which ends either if the player completes the level or loses their three lives after three unsuccessful attempts. The data corresponding to the 2 remaining, held-out levels (of the 22) were used as an out-of-distribution set to test the generalizability of the models.
